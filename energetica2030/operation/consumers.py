...
class MagangueTrip():
    ...
    def startparking(self, parkingID, motoNumber):
    ...
    def startparkingship(self, parkingIDship, shipNumber):
    ...
    def operationGraphics(self):
            
        Demanda_a = np.zeros(25)
        Demanda_a[12] = demanss_1[0][0]
        r1,r2,r3,r4,r5,r6,r7,r8,tim, tim_2 = self.QP(Demanda_a)
...
class SincelejoTrip():
    class moto:
        ...
        def __init__(self, moto_id_in, Parking_in):
            ...
            self.tripstate = []
            #self.speed= []
            self.speed = geek.zeros([tsim, 1], dtype = float)
            self.acel= []
            self.hybrid_cont= []
            self.new_route_2 = []
            self.new_route_n = []
            ...
            self.trip_c = 0
            self.State_bater = 3700
            self.Energy= geek.zeros([tsim, 1], dtype = float)
            self.Energy1= geek.zeros([tsim, 1], dtype = float)
            self.combus= geek.zeros([tsim, 1], dtype = float)
            self.combus2= geek.zeros([tsim, 1], dtype = float)
            self.E_TOTAL= geek.zeros([tsim, 1], dtype = float)
            self.moto_dist2 = geek.zeros([2, 1], dtype = int)
            self.pow_consumption = []
            self.pcn_consumption = []
            self.moto_dist1 = geek.zeros([2, 1], dtype = int)
        ...
        def E(self,moto_id_in,trips,step):
            ...
            if self.Energy[step-1] >= 3692 and (moto_id_in not in traci.parkingarea.getVehicleIDs("m3")):
                if trips == 1:
            ...
            elif self.Energy[step-1] < 3692 and (moto_id_in not in traci.parkingarea.getVehicleIDs("m3")):
                if trips == 1:
            ...
            if self.Energy[step-1] >= 3692 and (moto_id_in in traci.parkingarea.getVehicleIDs("m3")) and (self.Parking == 'm1' or self.Parking == 'm2'):
                self.Energy[step]= self.Energy[step-1] 
                self.State_bater = self.Energy[step]
                self.combus[step] = 0
                self.E_TOTAL[step] = 0  
...
class mapConsumer(WebsocketConsumer):
    ...
    def simulationMagangue(self, step, m_list, ch_ship, route):
        ...
        # ------------------ EMBARCACIÓN-------------------------------------------------------------------------------------------------
            ...
            step += 1
            x_on, x_at = traci.vehicle.getPosition('moto_0')
    ...
    def simulationSincelejo(self, step, m_list, route):
        ...
        # Barrio El Cortijo - Gobernación [89198373#3 ,88009100#2]
        # Altos del rosario - Plaza cultural de Majagual [89228441#4 ,295525947]

        or_m = ['89143440#5','323063346#13','89198373#3','89228441#4']
        des_m = ['323063346#13','295522603#2','88009100#2','295525947']
        ...
        while step < tsim:
            ...
            if step >= 100:
                ...
                if m_list != []:
                    for m_1 in m_list:
                        if motOBJ_dic[m_1].moto_state()[1] == 3:
                            if motOBJ_dic[m_1].Energy[step] >= 3692:#3430:
                                motOBJ_dic[m_1].moto_peti(4)
                            elif motOBJ_dic[m_1].Energy[step] < 3692:#3430: #3691
                                motOBJ_dic[m_1].moto_peti(5)
                            m_1_list.remove(m_1)
                    m_list = m_1_list
                ...
                for m_2 in motos_action:
                    if m_2[0:4] == 'moto':
                        if motOBJ_dic[m_2].trip_c == 4:
                            ...
                            d_m = dist_moto.astype(int) 
                            sort = sorted(d_m)
                            pos_0= numpy.where(d_m == sort[0])
                            pos_1= numpy.where(d_m == sort[1])
                            cnt_1 = parkOBJ_dic[parkingID[int(pos_0[0])]].count(parkingID[int(pos_0[0])]) 
                            cnt_2 = parkOBJ_dic[parkingID[int(pos_1[0])]].count(parkingID[int(pos_1[0])])
                            cnt_t = list(cnt_1[0]) + list(cnt_2[0])
                            c_1 = list(cnt_1)[1]
                ...
                if step % 1000 == 0: # la simulación comienza a los 100 segundos, entonces intervalo de 500 y 500
                    E_1 = list(tuple(reversed(traci.lane.getLastStepVehicleIDs ('-295445871#1_0'))) + tuple(reversed(traci.lane.getLastStepVehicleIDs ('295445871#1_0'))))
                    dif = set(motos_action_1).difference(set(E_1))

                    E_1 = list(tuple(reversed(traci.lane.getLastStepVehicleIDs ('-295445871#1_0'))) + tuple(dif)+ tuple(reversed(traci.lane.getLastStepVehicleIDs ('295445871#1_0'))))
        ...
        # ------------------CARGA--------------------------------------------------------------------------------------------------------------------------
                ...
                motos_action_11 = []
                for m_1_31 in moto_list:
                    if m_1_31[0:4] == 'moto':
                        if (motOBJ_dic[m_1_31].moto_state()[1] == 0 or motOBJ_dic[m_1_31].moto_state()[1] == 1) and (motOBJ_dic[m_1_31].Parking == 'm2' or motOBJ_dic[m_1_31].Parking == 'm1') and m_1_31 not in (list(traci.parkingarea.getVehicleIDs('m1'))+list(traci.parkingarea.getVehicleIDs('m2'))):
                            motos_action_11.append(m_1_31)
        ...
        # ------------------VIAJE DE ESTACIÓN A PUNTO DE ACOPIO--------------------------------------------------------------------------------------------
                for m_1_5 in motos_action_2:
                    if (motOBJ_dic[m_1_5].moto_state()[1] == 0 and motOBJ_dic[m_1_5].Parking == 'm3') and motOBJ_dic[m_1_5].Energy[step] == 3700:               
                        ...
                        d_m_1 = dist_moto_2.astype(int)
                        sort_1 = sorted(d_m_1)
                        pos_0_1= numpy.where(d_m_1 == sort_1[0])
                        pos_1_1= numpy.where(d_m_1 == sort_1[1])
                        cnt_1_1 = parkOBJ_dic[parkingID[int(pos_0_1[0])]].count(parkingID[int(pos_0_1[0])]) #cuenta los espacios disponibles en la estacion de carga
                        cnt_2_1 = parkOBJ_dic[parkingID[int(pos_1_1[0])]].count(parkingID[int(pos_1_1[0])]) 
                        cnt_t_0 = list(cnt_1_1[0]) + list(cnt_2_1[0])
                        ...
                        for m_6 in motos_action_11: 
                            if (motOBJ_dic[m_6].moto_state()[1] == 0 or motOBJ_dic[m_6].moto_state()[1] == 1 or motOBJ_dic[m_6].moto_state()[1] == 2 or motOBJ_dic[m_6].moto_state()[1] == 3) and  (motOBJ_dic[m_6].Parking == 'm1' or  motOBJ_dic[m_6].Parking == 'm2'):
                                if motOBJ_dic[m_6].Parking == parkingID[int(pos_0_1[0])]:
                                    c_1_1 = c_1_1 - i_1
                                if motOBJ_dic[m_6].Parking == parkingID[int(pos_1_1[0])]:
                                    c_2_1= c_2_1 - i_2
    ...
    def receive(self, text_data):
        ...
        route = text_data_json['route']

        if(city=='1' and route == '1'):
            tsim = 7000
        else:
            tsim = 2000
